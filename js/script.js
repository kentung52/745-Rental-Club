...
//  ===== Minimal JS: Gallery ===== 
		...
		(function () {
			...
			const mainLink = document.getElementById('galleryMainLink');
			const thumbsWrap = document.getElementById('galleryThumbs');
			const thumbs = thumbsWrap ? Array.from(thumbsWrap.querySelectorAll('.car-thumb')) : [];
			...
			function setActive(i) {
				...
				const url = thumbs[idx].getAttribute('data-full');
				if (url) {
					main.src = url;
	...
	// ===== Minimal JS: Refurb Slider ===== 

(function () {
  const root = document.getElementById('refurb');
  if (!root) return;

  const mainImg = document.getElementById('rfMainImg');
  const mainLink = document.getElementById('rfMainLink');
  const zoomBtn = document.getElementById('rfZoomBtn');

  const btnPrev = document.getElementById('rfPrev');
  const btnNext = document.getElementById('rfNext');

  const thumbsWrap = document.getElementById('rfThumbs');
  const thumbs = thumbsWrap ? Array.from(thumbsWrap.querySelectorAll('.rf-thumb')) : [];
  if (!thumbs.length) return;

  const elIdx = document.getElementById('rfIdx');
  const elTotal = document.getElementById('rfTotal');
  const elBar = document.getElementById('rfBar');
  const elStepLabel = document.getElementById('rfStepLabel');
  ...
  // 總數
  if (elTotal) elTotal.textContent = String(thumbs.length);

  // 放大查看 -> 觸發主圖連結（給 lightbox 用）
  if (zoomBtn && mainLink) {
  ...
  function setActive(i) {
    ...
    if (elIdx) elIdx.textContent = String(humanIdx);
    if (elStepLabel) elStepLabel.textContent = `Step ${humanIdx} / ${thumbs.length}`;
    if (elBar) elBar.style.width = `${Math.round((humanIdx / thumbs.length) * 100)}%`;
...
(function () {
  ...
  const els = {
    category: document.getElementById("dbxBudgetCategory"),
    model: document.getElementById("dbxBudgetModel"),
    ...
    rentalSubtotal: document.getElementById("dbxRentalSubtotal"),
  ...
  function getCategoryCars() {
    return dbxCars[els.category.value] || [];
  }

  function populateModels() {
    const cars = getCategoryCars();
    els.model.innerHTML = cars.map((car) => {
  ...
  function updateFeatured(car, billingDays, insuranceSum, total) {
    if (!car) {
      els.featured.innerHTML = `
        ...
        <h4>目前沒有可用車型</h4>
        <p>請先確認該分類是否已有車型資料。</p>
        <a href="/introduce.html">查看所有車款</a>
      `;
      return;
    }
  ...
  function updateBudget() {
    ...
    updateFeatured(car, billingDays, insuranceSum, total);
  }
  ...
  els.model.addEventListener("change", updateBudget);
